...
describe("Gram-Schmidt QR", function() {
  ...
  it('computes the in-place QR factorization of a square matrix',function() {
    var i,j;
    var n=3, m=3;
    var A = ndarray(new Float64Array([1,2,3,4,5,6,7,8,9]), [n,m]);
    ...
    gemm( QR, A, R ); 
    A = ndarray(new Float64Array([1,2,3,4,5,6,7,8,9]), [n,m]); // <-- the original has been overwritten, so reassign
    ops.sub( diff, QR, A );
    var err = ops.norm2( diff ); // <-- not to be confused with the 2-norm of a matrix, but it's basically good enough
    assert.closeTo( err, 0, 1e-8 );
